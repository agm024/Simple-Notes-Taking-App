...
def index():
    ...
    if request.method == 'POST':
        ...
        if note_content:
            ...
            notes.append({"content": note_content, "password": hashed_password})
            save_notes(notes)
...
def edit_note(note_id):
    ...
    if 0 <= note_id < len(notes):
        if request.method == 'POST':
            ...
            if new_content:
                # If new_password is provided, hash it; otherwise, set it to None
                if new_password:
        ...
        # Display the note in the edit form
